import { executeBenchmark, record, recordFixedValue } from './util/bench.js';
import {
...
async function runAll() {
    ...
    await executeBenchmark('Create ipv4 prefix pool 100x serially', {},
        async (histograms) => {
            const count = 100;
    ...
    await executeBenchmark('Create ipv4 prefix pool 100x parallelly', {},
        async (histograms) => {
            let resourceTypeId, strategyId;
            await record(histograms, 'setup',
                async () => {
    ...
    await executeBenchmark('allocate 100 ipv4_prefix_pool resources serially', {},
        async (histograms) => {
    ...
    await executeBenchmark('allocate 100 ipv4_prefix_pool resources parallelly', {},
        async (histograms) => {
            const iterations = 100;
            let poolId = await record(histograms, 'setup', async () => (await createIpv4PrefixRootPool()).id);
    ...
    await executeBenchmark('allocate 100 ipv4_pool resources serially', {},
        async (histograms) => {
            const iterations = 100;
    ...
    await executeBenchmark('allocate 100 ipv4_pool resources parallelly', {},
        async (histograms) => {
            const iterations = 100;
            const poolId = await record(histograms, 'setup', prepareIpv4Pool);
            await record(histograms, 'awaitPromisses',
