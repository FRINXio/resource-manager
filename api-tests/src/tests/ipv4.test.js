import { claimResources, getResourcePool, getPoolHierarchyPath } from '../graphql-queries.js';
import {
...
// test('ipv4_prefix_pool 100x serially', async (t) => {
//     const count = 100;
...
// test('ipv4_pool 100x serially', async (t) => {
//     const count = 100;
...
// test('ipv4_prefix_pool 100x parallelly', async (t) => {
...
// test('ipv4_pool 100x parallelly', async (t) => {
//     const count = 100, retries = 1000;
//     const poolId = await prepareIpv4Pool();
//     const createdIPs = (await allocateFromIPv4PoolParallelly(
...
async function allocateIPsUsingResourceCount(t, resourceCount) {
    const poolId = await prepareIpv4Pool();
    const createdIPs = (await claimResources(poolId, {resourceCount})).map(it => it.Properties.address).sort();
    const queriedIPs = (await queryIPs(poolId, resourceCount)).sort();
    t.deepEqual(queriedIPs, createdIPs);
    t.equal(queriedIPs.length, resourceCount);
}

test('allocate 100 IPs at once', async (t) => {
    await allocateIPsUsingResourceCount(t, 100);
    t.end();
});

test('allocate pool and 100 IPs at once x 2 in parallel', async (t) => {
    // FIXME: should not fail
    const parallelism = 2;
    const promises = [];
    for (let i = 0; i < parallelism; i++) {
        promises.push(allocateIPsUsingResourceCount(t, 100));
    }
